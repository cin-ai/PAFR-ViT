...
class Encoder(nn.Module):
    def __init__(self, config):
        ...
        self.feature_selector = FeatureSelector(hidden_dim=config.hidden_size)
    ...
    def forward(self, hidden_states):
        ...
        for i, layer in enumerate(self.layer):
            ...
            _, part_inx = self.part_select([weights])
            part_attn_weights.append(part_inx)
        feature_selector = self.feature_selector(hidden_states, part_attn_weights)
        part_states, part_weights = self.part_layer(feature_selector)
...
class FeatureReinforcement(nn.Module):
    def __init__(self, hidden_dim, alpha=0.3, dropout_prob=0.1):
        super().__init__()
        self.alpha = alpha
    ...
    def forward(self, hidden_states, part_attn_weights):
        B, L, D = hidden_states.shape
        adjusted_weights = [idx + 1 for idx in part_attn_weights]
        combined_indices = torch.cat(adjusted_weights, dim=1)  # [B, T]

        gathered = []
        for b in range(B):
            gathered.append(hidden_states[b][combined_indices[b]])  # [T, D]
        selected_features = torch.stack(gathered, dim=0)  # [B, T, D]

        attn_scores = self.linear1(selected_features)
        attn_scores = self.tanh(attn_scores)
        attn_scores = self.dropout(attn_scores)
        attn_scores = self.layer2(attn_scores).squeeze(-1)  # [B, T]
        attn_weights = F.softmax(attn_scores, dim=1)  # [B, T]
        attn_weights = self.dropout(attn_weights)

        attn_output = torch.einsum('btd,bt->bd', selected_features, attn_weights)
        attn_output = attn_output.unsqueeze(1).expand(-1, L, -1)  # [B, L, D]

        return self.norm(hidden_states + self.alpha * attn_output)
